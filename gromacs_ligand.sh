#!/bin/bash
# Script for handling MD in GROMACS - J.D. Colburn 23/10/18
...
#get name
intro
printf ' > Please choose protein: '											#asks for name of peptide
read PDB																	#the name you type in is saved as the variable "$PDB"
printf ' > Please choose ligand: '											#asks for name of ligand
read LIGAND	
...
echo '  the setup for this system (i.e. the minimisation) ' 
echo ''
...
#cp $LIGNDIR/$LIGAND.pdb $EMDIR/$LIGAND.pdb
cp $LIGNDIR/$LIGAND.itp $EMDIR/$LIGAND.itp
#cp $LIGNDIR/$LIGAND.top $EMDIR/$LIGAND.top
cp $LIGNDIR/$LIGAND.gro $EMDIR/$LIGAND.gro

if [ -a $INITDIR/$PDB.pdb ]; then #make .gro:
echo " +-----------------+"
echo " | > PDB to GMX    |"
echo " +-----------------+"
$GROMDIR/pdb2gmx -f $EMDIR/$PDB.pdb -ignh -asp -glu -his -o $EMDIR/$PDB\_read.gro -p $EMDIR/topol.top -water tip3p
#$GROMDIR/pdb2gmx -f $EMDIR/$PDB.pdb -ignh -o $EMDIR/$PDB\_read.gro -p $EMDIR/topol.top -water tip3p

echo ''
# Fix the new combined geometry file

echo " Combining PDB files for protein and ligand..." 
#cat $EMDIR/$LIGAND.gro >> $EMDIR/$PDB\_read.gro								# Make combined PDB file
#sed -i '/ENDMDL/d' 		  $EMDIR/$PDB.pdb			
tail -1 $EMDIR/$PDB\_read.gro > $EMDIR/boxvector.tmp							# lastline of gro file
sed -i '$d' $EMDIR/$PDB\_read.gro
cat $EMDIR/$LIGAND.gro | sed '1,2d' | sed '$d' >> $EMDIR/$PDB\_read.gro
cat $EMDIR/boxvector.tmp >> $EMDIR/$PDB\_read.gro
rm $EMDIR/boxvector.tmp

# Fix up the modified topology	
echo " Adding include statements to topology" 
sed -i "/forcefield.itp/a #include \"$LIGAND.itp\"" $EMDIR/topol.top		# add include statement
MOLECULE="$(grep -A1 '; Compound        nmols' $EMDIR/$LIGAND.top | tail -1)"							
echo $MOLECULE >> $EMDIR/topol.top											# add molecule to topol

# fix the discrepancy in atom numbers
echo ''
ATMNO="$(sed '2,2!d' $EMDIR/$PDB\_read.gro)"
echo " Total atoms in .gro file: $ATMNO "
LIGATMNO="$(grep -o 'ATOM' $EMDIR/$LIGAND.pdb | wc -l)"
echo " Additional ligand atoms: $LIGATMNO "
NEWNO=$(($ATMNO + $LIGATMNO))
echo " New total: $NEWNO "
echo " Editing .gro file "
sed -i "2s/$ATMNO/$NEWNO/" $EMDIR/$PDB\_read.gro
...
do
	START=$SNAP
	END=$SNAP
	$GROMDIR/trjconv -f $MDDIR/$PDB\_md.trr -s $MDDIR/$PDB\_md.tpr -pbc mol -o $SNAPDIR/$PDB\_$SNAP.pdb -b $START -e $END	<<< 1								 
done
